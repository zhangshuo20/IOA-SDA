...
% guangdong_data_EEIO_2000_2015.mat中的变量guangdong_EEIO_2000_2015为元胞数组，共7个元素，均为46*49（含有进口+调出列）的EEIO矩阵,第43-46行分别对应CO2,SO2,NOx和PM；
% guangdong_EEIO_2000_2015{i},i=1-7分别对应年份为2000,2002,2005,2007,2010,2012,2015
for i=1:length(guangdong_EEIO_2000_2015)-1
%    MIOT_0=guangdong_EEIO_2000_2015{i}(1:43,:);%CO2
%    MIOT_1=guangdong_EEIO_2000_2015{i+1}(1:43,:);%CO2
    MIOT_0=[guangdong_EEIO_2000_2015{i}(1:42,:);guangdong_EEIO_2000_2015{i}(44,:)];%SO2
    MIOT_1=[guangdong_EEIO_2000_2015{i+1}(1:42,:);guangdong_EEIO_2000_2015{i+1}(44,:)];%SO2
%    MIOT_0=[guangdong_EEIO_2000_2015{i}(1:42,:);guangdong_EEIO_2000_2015{i}(45,:)];%NOx
%    MIOT_1=[guangdong_EEIO_2000_2015{i+1}(1:42,:);guangdong_EEIO_2000_2015{i+1}(45,:)];%NOx
%    MIOT_0=[guangdong_EEIO_2000_2015{i}(1:42,:);guangdong_EEIO_2000_2015{i}(46,:)];%PM
%    MIOT_1=[guangdong_EEIO_2000_2015{i+1}(1:42,:);guangdong_EEIO_2000_2015{i+1}(46,:)];%PM
...
year=[2000,2002,2005,2007,2010,2012,2015];
year=year';
...
% 元胞数组sEa_chaining与元胞数组guangdong的长度相同，i=1-7分别对应年份为2000,2002,2005,2007,2010,2012,2015
...
sheetname={'2000','2002','2005','2007','2010','2012','2015'};%建立数据年份标签，与guangdong数据对应
...
for i=1:length(guangdong_EEIO_2000_2015)       
    ...
    title={'EP intensity','Leontief inverse','final demand structure','final demand composition','per capita final demand','population'};% 添加变量名称
    result= [title; data_cell];                         % 将变量名称和数值组集到result
    s1=xlswrite('C:\Users\yuyd\Desktop\广东CO2-SDA\results_SO2.xlsx',result,sheetname{i},'L3:Q45');% 将sEa_chaining的结果写入到文件对应位置   
end

s2=xlswrite('C:\Users\yuyd\Desktop\广东CO2-SDA\results_SO2.xlsx',Ea_DL,'SDA-LMDI&DL','D9:I15');% 将Ea_DL的结果写入到文件对应位置 
s3=xlswrite('C:\Users\yuyd\Desktop\广东CO2-SDA\results_SO2.xlsx',Ea_DL_chaining,'SDA-LMDI&DL','N9:S15');% 将Ea_DL_chaining的结果写入到文件对应位置
s2=xlswrite('C:\Users\yuyd\Desktop\广东CO2-SDA\results_SO2.xlsx',Ea_LMDI,'SDA-LMDI&DL','D25:I31');% 将Ea_LMDI的结果写入到文件对应位置
s3=xlswrite('C:\Users\yuyd\Desktop\广东CO2-SDA\results_SO2.xlsx',Ea_LMDI_chaining,'SDA-LMDI&DL','N25:S31');% 将Ea_LMDI_chaining的结果写入到文件对应位置
